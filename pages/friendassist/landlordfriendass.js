...
Page({
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (value.code == 0 && value.data.hasActive == 1) {
          ...
          } else if (value.data.totalMoney > 6 && value.data.totalMoney < 20) {
            that.setData({
              viewBg1: '100%',
              viewBg2: '50%',
              viewBg3: '0%',
              reachmoney1: 'color:#62350E;background:#fff',
              giftname: value.data.goods.flexGoods[0]
            });
          } else if (value.data.totalMoney > 20 && value.data.totalMoney < 100) {
            that.setData({
              viewBg1: '100%',
              viewBg2: '100%',
              viewBg3: '50%',
              reachmoney2: 'color:#62350E;background:#fff',
              giftname: value.data.goods.flexGoods[1]
            });
          } else if (value.data.totalMoney > 100) {
            that.setData({
              viewBg1: '100%',
              viewBg2: '100%',
              viewBg3: '100%',
              reachmoney3: 'color:#62350E;background:#fff',
  ...
  bindGetUserInfo: function (e) {
    ...
    if (e.detail.encryptedData) {
      if (that.options && that.options.zg_id > 0) {
        var active = { id: that.options.id, active_name: '欢乐斗地主', zg_id: that.options.zg_id };
  ...
  assistgift: function (e) {
    var that = this;
    wx.redirectTo({
      url: '/pages/myselfassist/landlordassist?id=' + that.data.activityid,
    });
  },
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      title: '点一下，你和TA都能获得欢乐豆奖励！',
      path: 'pages/index/index',
      desc: '',
      imageUrl: 'https://qmzgcdn.boc7.net/test/landlord_share2.jpg'
    }
  }
})