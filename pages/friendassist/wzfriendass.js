...
Page({
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (value.code == 0 && value.data.hasActive == 1) {
          ...
          } else if (value.data.totalMoney > 6 && value.data.totalMoney < 28.8) {
            that.setData({
              viewBg1: '100%',
              viewBg2: '50%',
              viewBg3: '0%',
              reachmoney1: 'color:#4C3372;background:#fff',
              giftname: value.data.goods.flexGoods[0]
            });
          } else if (value.data.totalMoney > 28.8 && value.data.totalMoney < 168.8) {
            that.setData({
              viewBg1: '100%',
              viewBg2: '100%',
              viewBg3: '50%',
              reachmoney2: 'color:#4C3372;background:#fff',
              giftname: value.data.goods.flexGoods[1]
            });
          } else if (value.data.totalMoney > 168.8) {
            that.setData({
              viewBg1: '100%',
              viewBg2: '100%',
              viewBg3: '100%',
              reachmoney3: 'color:#4C3372;background:#fff',
  ...
  bindGetUserInfo: function (e) {
    ...
    if (e.detail.encryptedData) {
      if (that.options && that.options.zg_id > 0) {
        var active = { id: that.options.id, active_name: '王者荣耀', zg_id: that.options.zg_id };
  ...
  assistgift: function (e) {
    var that = this;
    wx.redirectTo({
      url: '/pages/myselfassist/wzassist?id=' + that.data.activityid,
    });
  },
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      ...
      desc: '',
      imageUrl: 'https://qmzgcdn.boc7.net/test/wz_share8.jpg'
    }
  }
})