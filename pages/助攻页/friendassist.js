...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.zg_id > 0) {
      ...
      var data = { token: token, active_id: that.data.activityid, zg_id: that.data.zhugongid };//token字符串转成对象
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (value.code == 0 && value.data.hasActive == 1) {
            ...
            if (token) {
              that.setData({
                hashelps: value.data.user.hasHelp,
                friendassistcoin: value.data.user.help_money
              });
            }

            if ((options.active_name == '王者荣耀' && value.data.totalMoney < 6) || (options.active_name == '穿越火线' && value.data.totalMoney < 6) || (that.options.active_name == '海盗来了' && value.data.totalMoney < 8)) {
              that.setData({
                viewBg1: '50%',
                viewBg2: '0%',
                viewBg3: '0%'
              });
            } else if ((options.active_name == '王者荣耀' && value.data.totalMoney > 6 && value.data.totalMoney < 28.8) || (options.active_name == '穿越火线' && value.data.totalMoney > 6 && value.data.totalMoney < 68) || (that.options.active_name == '海盗来了' && value.data.totalMoney > 8 && value.data.totalMoney < 28)) {
              that.setData({
                viewBg1: '100%',
                viewBg2: '50%',
                viewBg3: '0%',
            ...
            } else if ((options.active_name == '王者荣耀' && value.data.totalMoney > 28.8 && value.data.totalMoney < 88.8) || (options.active_name == '穿越火线' && value.data.totalMoney > 68 && value.data.totalMoney < 408) || (that.options.active_name == '海盗来了' && value.data.totalMoney > 28 && value.data.totalMoney < 50)) {
              that.setData({
                viewBg1: '100%',
                viewBg2: '100%',
                viewBg3: '50%',
            ...
            } else if ((options.active_name == '王者荣耀' && value.data.totalMoney > 88.8) || (options.active_name == '穿越火线' && value.data.totalMoney > 408) || (that.options.active_name == '海盗来了' && value.data.totalMoney > 50)) {
              that.setData({
                viewBg1: '100%',
                viewBg2: '100%',
                viewBg3: '100%',
  ...
  bindGetUserInfo: function (e) {
    ...
    if (e.detail.encryptedData) {
      if (that.options && that.options.zg_id > 0) {
        var active = { id: that.options.id, active_name: that.options.active_name, zg_id: that.options.zg_id };
  ...
  AssistingFriends: function (e) {
    ...
    if(token){
      ...
      if (that.options){
        ...
        // wx.navigateTo({
        //   url: '/pages/login/login?id=' + that.data.activityid + '&&active_name=' + that.data.activename + '&&zg_id=' + that.data.zhugongid
        // })
      }else{
        wx.navigateTo({
          url: '/pages/login/login'
  ...
  assistgift: function (e) {
    var that = this;
    wx.redirectTo({
      url: '/pages/myselfassist/myselfassist?id=' + that.data.activityid + '&&active_name=' + that.data.activename,
    });
  },
