...
Page({
  ...
  data: {
    ...
    condition:2,//宝箱兑换条件，1为不满足，2为是否兑换，3为兑换成功，4为已经兑换过
  ...
  onLoad: function (options) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (value.code == 0) {
          if (value.data.advertise.length > 0) {
            that.setData({
              gamelist: value.data.advertise
            });
          }
          ...
          if (token) {
            ...
            if ((options.active_name == '王者荣耀' && value.data.totalMoney < 6) || (options.active_name == '穿越火线' && value.data.totalMoney < 6) || (options.active_name == '海盗来了' && value.data.totalMoney < 8)) {
              that.setData({
                viewBg1: '50%',
                viewBg2: '0%',
                viewBg3: '0%'
              });
            } else if ((options.active_name == '王者荣耀' && value.data.totalMoney > 6 && value.data.totalMoney < 28.8) || (options.active_name == '穿越火线' && value.data.totalMoney > 6 && value.data.totalMoney < 68) || (options.active_name == '海盗来了' && value.data.totalMoney > 8 && value.data.totalMoney < 28)) {
              that.setData({
                viewBg1: '100%',
                viewBg2: '50%',
                viewBg3: '0%',
                reachmoney1: 'color:#201A4F;background:#fff',
                giftname: value.data.goods.flexGoods[0]
              });
            } else if ((options.active_name == '王者荣耀' && value.data.totalMoney > 28.8 && value.data.totalMoney < 88.8) || (options.active_name == '穿越火线' && value.data.totalMoney > 68 && value.data.totalMoney < 408) || (options.active_name == '海盗来了' && value.data.totalMoney > 28 && value.data.totalMoney < 50)) {
              that.setData({
                viewBg1: '100%',
                viewBg2: '100%',
                viewBg3: '50%',
                reachmoney2: 'color:#201A4F;background:#fff',
                giftname: value.data.goods.flexGoods[1]
              });
            } else if ((options.active_name == '王者荣耀' && value.data.totalMoney > 88.8) || (options.active_name == '穿越火线' && value.data.totalMoney > 408) || (options.active_name == '海盗来了' && value.data.totalMoney > 50)) {
              that.setData({
                viewBg1: '100%',
                viewBg2: '100%',
                viewBg3: '100%',
                reachmoney3: 'color:#201A4F;background:#fff',
            ...
            if (value.data.hasActive == 1) {
              that.setData({
                zg_id: value.data.user.zg_id
              });
            }
  ...
  generate: function (e) {
    ...
    var token = wx.getStorageSync('token');
    var getuserid = { token: token };//token字符串转成对象
    getuserid = app.encrypt(JSON.stringify(getuserid));//对象转成字符串加密后
    wx.request({
      url: 'https://qmzg.boc7.net/getUid',
      method: 'post',
      data: { data: getuserid },//字符串转成json对象
      success: function (res) {
        that.setData({
          userid: res.data
        });
        ...
        success: function (res) {
          ...
          if (value.code == 0) {
            if (value.time == 0) {
              ...
              that.setData({
                buffertime: ',' + hours + ':' + minutes + ':' + seconds + '后可以再次发起助攻'
              })
            }
            if (that.data.exchangetype == 2) {
              that.setData({
                giftname: { id: that.data.giftname.id, goods_name: value.goods_name, money: that.data.giftname.money },
                buffertime: ''
              })
            }
            that.setData({
...
/**
  ...
  onShareAppMessage: function (res) {
    ...
    if (that.data.activename == '王者荣耀'){
      return {
        title: '点一下，你和TA都能获得王者皮肤点券等奖励！',
        path: 'pages/friendassist/friendassist?id=' + that.options.id + '&&active_name=' + that.data.activename + '&&zg_id=' + that.data.zg_id,
        desc: '',
        imageUrl: 'https://qmzgcdn.boc7.net/test/wz_share9.jpg'
      }
    }else if (that.data.activename == '穿越火线') {
      return {
    ...
    }else if (that.data.activename == '海盗来了') {
      return {
